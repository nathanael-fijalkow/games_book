...
\begin{tikzpicture}[scale=1.3]
    ...
    (v3) edge node[right] {$\frac 1 2$} (v7)%    (init) edge node[left] {$\frac 3 {10}$} (rv)    ;    % \node[s-random] (left) at (-.75,-1) {};% \node[s-adam] (lleft) at (-1.5,-2) {$v_1$};% \node[s-eve] (rright) at (.75,-2) {$v_3$};
% (root) edge node[right] {$\frac 3 5$}  (right)% (left) edge node[right] {$\frac 3 4$} (rleft)% (right) edge[out=70,in=0] node[right] {$\frac 1 3$} (root)%%%%% ;
    \end{tikzpicture}
\end{document}