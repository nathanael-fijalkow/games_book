...
%\path[->] (A2) edge node[above] {$1$} (A3);%\path[->] (A3) edge node[above] {$1$}(A4);%%\path[->] (A3) edge node[above] {$1$} (E2);%\path[->,dotted] (A5) edge node[above] {$1$}(E4);%\path[->] (E0) edge[bend right] node[below] {$0$} (E1);%\path[->] (E2) edge node[above] {$2$} (E1);%\path[->] (E4) edge node[above] {$2$} (E3);
\path[->] (A1) edge    (A2);
\path[->] (A2) edge  (A3);
\path[->] (A2) edge (A3);
\path[->] (A3) edge (A4);
