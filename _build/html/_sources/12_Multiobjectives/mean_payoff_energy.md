...
e., prefix of play) $\rho$ starting in vertex $v$ and ending in vertex $v_\eta$, we add children as follows:
    ...
    *  otherwise $\eta$ has one child for each possible successor of $v_\eta$, i.e., for each $Out(e)$ such that $e \in E$ and $In(e) = v_\eta$.
...
\tau(v) =
\begin{cases}
\tau_Pre(v) &\text{if } v \in W^{i,  Pre}_{\text{Adam}} \setminus W^{i}_{\text{Adam}},\\
...
The restriction $\mathcal{A}' \downharpoonright V'$ defines a proper arena because $W^i_{\text{Adam}} =  Attr_\mathrm{Adam}(W^i_{\text{Adam}})$. Indeed, any vertex $v$ from which Adam can force to reach $W^i_{\text{Adam}}$ also belongs to $W^i_{\text{Adam}}$ by prefix independence of the mean-payoff.
