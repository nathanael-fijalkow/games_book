...
infinite arena $\mathcal{A}_\mathbb{N}(\mathcal{V})=(V,E, V_\mathrm{Eve}, V_\mathrm{Adam})$: $\tau$ maps vertices
in $V$ to edges in $E$.  We call $\tau$ counterless if, for all
locations $\ell\in \mathcal{L}_\mathrm{Adam}$ and all vectors
$\vec v,\vec v'\in\mathbb{N}^ k$, $\tau( \ell(\vec v))=\tau( \ell(\vec v'))$.
...
:label: 11-lem:counterless
  ...
  Let $\mathcal{V}=( \mathcal{L}, A, \mathcal{L}_\mathrm{Eve}, \mathcal{L}_\mathrm{Adam}, k)$ be an asymmetric
  vector system, $\ell_0\in \mathcal{L}$ be a location, and
  $\mathrm{lcol}{:}\, \mathcal{L}\to\{1,\dots,d\}$ be a location colouring.  If Adam 
  wins from $\ell_0(\vec v)$ for every initial credit $\vec v$ in the
  parity game played over $\mathcal{V}$ with $\mathrm{lcol}$, then
...
:class: dropdown tip
  ...
  2| \mathcal{L}_\mathrm{Adam}|$.  We proceed by induction over $| A_\mathrm{Adam}|$.  For
  the base case, if $| A_\mathrm{Adam}|=| \mathcal{L}_\mathrm{Adam}|$ then every location
  ...
  If none of those plays visits $\hat \ell$, then $\textrm{Eve}$ wins in $\mathcal{V}$
  ...
  $\vec v_0  \stackrel{\!\,\!\,\textrm{def}}{=}\vec v_l+\hat{\vec v}$ and show that Eve wins from
  $\ell_0(\vec v_0)$.

  \begin{scope}    We define now a strategy $\sigma$ for $\textrm{Eve}$ over $\mathcal{V}$ that
    ...
    is $m(1)  \stackrel{\!\,\!\,\textrm{def}}{=} l$ for the initial segment and otherwise
    $m(j)  \stackrel{\!\,\!\,\textrm{def}}{=} l$ if $e_{j-1}=a_l$ and $m(j)  \stackrel{\!\,\!\,\textrm{def}}{=} r$ otherwise.  The
    $l$-subsequence associated with $\pi$ is the sequence of segments
    $\pi(l)  \stackrel{\!\,\!\,\textrm{def}}{=}\pi_{l_1}a_{l_2-1}\pi_{l_2}a_{l_3-1}\pi_{l_3}\cdots$
    with mode $m(l_i)=l$, while the $r$-subsequence is the sequence
    $\pi(r)  \stackrel{\!\,\!\,\textrm{def}}{=}\hat\pi a_{r_1-1}\pi_{r_1}a_{r_2-1}\pi_{r_2}\cdots$
    with mode $m(r_i)=r$ prefixed by $\hat\pi$.  Then we let
    $\sigma(\pi)  \stackrel{\!\,\!\,\textrm{def}}{=}\sigma_{m}(\pi(m))$ where $m\in\{l,r\}$ is the
...
Let $\mathcal{V}=( \mathcal{L}, A, k)$ be a vector addition system with states,
$\mathrm{lcol}{:}\, \mathcal{L}\to\{1,\dots,d\}$ a location colouring, and
$\ell_0\in \mathcal{L}$ an initial location.  Then Eve wins the
...
of $\mathcal{V}$ by following $\hat\pi$, and then $\ell(\vec v_0+\vec
...
$w(\Pi)  \stackrel{\!\,\!\,\textrm{def}}{=}\sum_{\pi\in\Pi} w(\pi)$ be its weight; we write
...
:label: 11-lem:zmulticycle
  ...
  Let $\mathcal{V}$ be a vector addition system with states,
  $M\in 2^{ A}$, and $F\subseteq A$.  We can check in polynomial
  time whether $\mathcal{V}$ contains a non-negative multi-cycle $\Pi$
  suitable for $(M,F)$.
...
$$
    ...
    \forall \ell&\in \mathcal{L},&\sum_{a\in\mathrm{in}( \ell)}x_a&=\sum_{a\in\mathrm{out}( \ell)}x_a\;,\tag{multi-cycle}\\
...
Let us define for this the relation $\ell\sim \ell'$ over $\mathcal{L}$ if
...
Thus $\sim$ defines a partition $\mathcal{L}/{\sim}$ of $\mathcal{L}$.
In order to find a non-negative cycle $\pi$ suitable for $(M,F)$,
we are going to compute the partition $\mathcal{L}/{\sim}$ of $\mathcal{L}$
...
$(P, A\cap(P\times\mathbb{Z}^ k\times P), k)$ induced by the equivalence
classes $P\in \mathcal{L}/{\sim}$.  This yields {numref}`11-algo:zcycle`, which
assumes that we know how to compute the partition $\mathcal{L}/{\sim}$.  Note
that the depth of the recursion in {numref}`11-algo:zcycle` is bounded
by $| \mathcal{L}|$ and that recursive calls operate over disjoint subsets
of $\mathcal{L}$, thus assuming that we can compute the partition in
...
:name: 11-algo:zcycle
:align: center
$\text{cycle}(\mathcal{V},M,F)$
```

It remains to see how to compute the partition $\mathcal{L}/{\sim}$. Consider
for this the set of actions
$A'  \stackrel{\!\,\!\,\textrm{def}}{=}\{a\mid\exists\Pi\text{ a non-negative multi-cycle
  suitable for $(M,F)$ with $a\in\Pi$}\}$ and
$\mathcal{V}'=( \mathcal{L}', A', k)$ the subsystem induced by $A'$.
\begin{claim}
\label{11-cl:part}
  There exists a path from $\ell$ to $\ell'$ in $\mathcal{V}'$
...
:class: dropdown tip
  ...
  in $A'$ and there is a path in $\mathcal{V}'$.  
...
strongly connected components of $\mathcal{V}'$.  This yields the following
polynomial time algorithm for computing $\mathcal{L}/{\sim}$.

```{figure} ./../FigAndAlgos/11-algo:part.png
:name: 11-algo:part
:align: center
$\text{partition}(\mathcal{V},M,F)$
```
...
:label: 11-lem:zcycle
  ...
  Let $\mathcal{V}$ be a vector addition system with states,
  $M\in 2^{ A}$, and $F\subseteq A$.  We can check in polynomial
  time whether $\mathcal{V}$ contains a non-negative cycle $\pi$
  suitable for $(M,F)$.
...
:class: dropdown tip
  ...
  Let $\mathcal{V}=( \mathcal{L}, A, k)$ be a vector addition system with states,
  $\mathrm{lcol}{:}\, \mathcal{L}\to\{1,\dots,$ $d\}$ a location colouring, and
  $\ell_0\in \mathcal{L}$ an initial location.  We start by trimming $\mathcal{V}$ to
  ...
  to set $M  \stackrel{\!\,\!\,\textrm{def}}{=}\{ \mathrm{lcol}^{-1}(p)\}$ and
  $F  \stackrel{\!\,\!\,\textrm{def}}{=} \mathrm{lcol}^{-1}(\{p+1,\dots,d\})$.
...
:class: dropdown tip
  ...
  $\mathcal{V}=( \mathcal{L}, A, \mathcal{L}_\mathrm{Eve}, \mathcal{L}_\mathrm{Adam}, k)$ and a location
  colouring $\mathrm{lcol}{:}\, \mathcal{L}\to\{1,\dots,2d\}$.
  By {prf:ref}`11-lem:parity2bounding`, the parity vector game with
  existential initial credit over $\mathcal{V}$ problem reduces to a
  bounding game with existential initial credit over a vector
  system $\mathcal{V}'=( \mathcal{L}', A', \mathcal{L}'_\mathrm{Eve}, \mathcal{L}'_\mathrm{Adam}, k+d)$ where
  $\mathcal{L}'\in O(| \mathcal{L}|)$ and $\| A'\|=\| A\|$.
  ...
  the bounded semantics $\mathcal{A}_B(\mathcal{V}')$ where $B$ is in
  $(| \mathcal{L}'|\cdot\| A'\|)^{O( k+d)^3}$.  Such a game can be solved in
  linear time in the size of the bounded arena using attractor
  techniques, thus in $O(| \mathcal{L}|\cdot B)^{ k+d}$, which is in
  $(| \mathcal{L}|\cdot\| A\|)^{O( k+d)^4}$ in terms of the original instance.
...
:class: dropdown tip
  ...
  $X=\{x_1,\neg x_1,x_2,$ $\neg x_2,\dots,x_k,\neg x_k\}$, we construct
  an asymmetric vector system $\mathcal{V}$ where Eve wins the
...
$$
    ...
    \vec u_\ell  \stackrel{\!\,\!\,\textrm{def}}{=}\begin{cases}
...
$$
  ...
  We define $\mathcal{V}  \stackrel{\!\,\!\,\textrm{def}}{=}( \mathcal{L}, A, \mathcal{L}_\mathrm{Eve}, \mathcal{L}_\mathrm{Adam},2k)$ where
...
(Lipton in {cite}`Lipton:1976`) of $\textrm{EXPSPACE}$-hardness of coverability in
the one-player case.
...
:class: dropdown tip
  ...
  location $\ell_0\in \mathcal{L}$, whether Eve has a winning strategy to
  visit $\ell_\mathtt{halt}(\vec v)$ for some $\vec v\in\mathbb{N}^ k$ from
...
```
  ...
  $\mathcal{V}=( \mathcal{L}', A', \mathcal{L}'_\mathrm{Eve}, \mathcal{L}_\mathrm{Adam}, k')$ with
  $k'=2 k+O(n)$.  Each of the counters $\mathtt{c}_i$ of $\mathcal{M}$ is
  paired with a **complementary** counter $\bar{\mathtt{c}_i}$ such
  that their sum is $B$ throughout the simulation of $\mathcal{M}$.  We
...
```
  ...
  on $\mathcal{V}$ starting from $\ell'_0(\vec 0)$ and with target
 ...
 Regarding non-termination games, we use essentially the
  ...
  can do so after at most $| \mathcal{L}|B^ k$ steps.  We therefore use a
  time budget: this is an additional component in $\mathcal{V}$ with
...
alternating Minsky machine, we need $O(|\mathcal{M}|)$ additional counters
...
:class: dropdown tip
  ...
  $\mathcal{V}=( \mathcal{L}, A, \mathcal{L}_\mathrm{Eve}, \mathcal{L}_\mathrm{Adam},1)$ with initial
  ...
  in $\mathcal{V}$.  The translation is depicted in {numref}`11-fig:dim2`.
...
```
  ...
  of $\mathcal{V}$ is simulated by a configuration $\ell(c,n_0-c)$ in $\mathcal{V}'$.
  ...
  according to the natural semantics of $\mathcal{V}$, if $c<n$ there should
