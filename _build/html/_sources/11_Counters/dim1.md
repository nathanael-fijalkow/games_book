...
```
 ...
 \Cref{11-th:undec} leaves open whether vector games might be
...
:class: dropdown tip
  ...
  positive.  Let $m  \stackrel{\!\,\!\,\raisebox{-.15ex}{\scalebox{.5}{\textrm{def}}}}{=} 2^{p(n)}\geq n$.  The computation of $\?M$
...
```
  ...
  $\Gamma'  \stackrel{\!\,\!\,\raisebox{-.15ex}{\scalebox{.5}{\textrm{def}}}}{=} \{ \triangleright, \triangleleft\}\cup\Gamma\cup(Q\times\Gamma)$. Rather
  than exhibiting the full computation from {numref}`11-fig:exp`, the
  game will be played over positions $(i,j,\gamma_{i,j})$ where
  $0<i\leq m$, $0\leq j\leq m+1$, and $\gamma_{i,j}\in\Gamma'$.   \textrm{Eve}\ 
  ...
  and $a\in\Gamma$ of  \textrm{Eve}'s choosing.  It should be clear that  \textrm{Eve}\ 
  ...
  countdown system $\?V  \stackrel{\!\,\!\,\raisebox{-.15ex}{\scalebox{.5}{\textrm{def}}}}{=}( \?L, A, \?L_\mathrm{Eve}, \?L_\mathrm{Adam},1)$.
  ...
  configuration $\ell_0(n_0)$ where $n_0  \stackrel{\!\,\!\,\raisebox{-.15ex}{\scalebox{.5}{\textrm{def}}}}{=} m\cdot(m+2)+1$ and the
  target location is $\smiley$.

  We define for this the sets of locations
  ```{math}

     \?L_\mathrm{Eve}&  \stackrel{\!\,\!\,\raisebox{-.15ex}{\scalebox{.5}{\textrm{def}}}}{=}\{ \ell_0,\smiley,\frownie\}
...
```
  ...
  that  \textrm{Eve}\ can reach $\smiley$ from a configuration $\ell_{=j}(c)$ if
...
:class: dropdown tip
   ...
   adding a new location $\ell'_0$ controlled by  \textrm{Eve}\ with actions
   $\ell'_0\step{1} \ell'_0$ and $\ell'_0\step 0 \ell_0$, and asking
   whether  \textrm{Eve}\ has a winning strategy starting from $\ell'_0(0)$ in
   ...
   of the countdown game.  \qedhere%%   Indeed,  \textrm{Eve}\ wins the countdown game
...
\decpb[zero reachability with given initial credit]
  ...
  $n_0\in\+N$.}
  {Does  \textrm{Eve}\ have a strategy to reach a configuration $\ell(0)$ for
  ...
  $\textsf{col}(e)= \textrm{Win}$ if and only if $\textrm{In}(e)= \ell(0)$ for some $\ell\in \?L$?}
...
:class: dropdown tip
  ...
  new initial location $\ell'_0$ controlled by  \textrm{Eve}\ with actions
  ...
  1 \ell'_0$ does not work, because  \textrm{Eve}\ would have a trivial winning
