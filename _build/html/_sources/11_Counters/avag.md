...
> **Asymmetric Games**
 ...
 A vector system
$\?V=(\?LA\Loc_\mathrm{Eve}\Loc_\mathrm{Adam}k$ is
asymmetric\index{asymmetry|see{vector system}} if, for all
locations $\ellin\Loc_\mathrm{Adam} controlled by \textrm{Adam} and all actions
$(\ellstep{\vec u}\ell)\inA originating from those,
...
:label: 11-ex:avg
  ...
  dimension two with locations partitioned as $\Loc_\mathrm{Eve}\{\ell\loc_{2,1},\loc_{\text-1,0}\}$ and $\Loc_\mathrm{Adam}\{\ell\}$
  ...
  be reached by playing the action $\ell\step{0,0}\loc_{\text-1,0}$,
...
:class: dropdown tip
  ...
  $\?M=(\?LAk$ and an initial location $\loc_0$ where we
  ...
  an asymmetric vector system
  $\?V\eqby{def}\?Luplus\Loc_{\eqby?0}\uplus\Loc_{k,A,\?Luplus\?L_{\eqby?0},\Loc_{\eqby?0},k$
  where all the original locations and $\Loc_{k$ are
  controlled by~\textrm{Eve} and $\Loc_{\eqby?0}$ is controlled by~\textrm{Adam}
...
```
  ...
  We
  use $\Loc_{\eqby?0}$ and $\Loc_{k$ as two sets of locations disjoint from $\?L defined by
  \begin{align*}
    \Loc_{\eqby?0}&\eqby{def}{\ell_{i\eqby?0}\in\?Ltimes\{1,\dots,k}\mid\exists\ellin\?Lmathbin.(\ellstep{i\eqby?0}\ell)\inA}\\
    \Loc_{k&\eqby{def}{\loc_{i}\mid 1\leq i\leq k}
    ...
    &\:\cup\:\{\ellstep{\vec
      0}\ell_{i\eqby?0},\;\;\:\ell_{i\eqby?0}\!\!\step{\vec
      0}\ell,\;\;\:\ell_{i\eqby?0}\!\!\step{\vec 0}\loc_{i}\mid
      (\ellstep{i\eqby?0}\ell)\inA}\\
    &\:\cup\:\{\loc_i\!\step{-\vec e_j}\loc_{i},\;\;\:\loc_{i}\!\step{\vec
      0}\loc_\mathtt{halt}\mid 1\leq i,j\leqk j\neq i\}\;.
  \end{align*}
  ...
  $\ellstep{\vec 0}\ell_{i\eqby?0}$.  Then, either \textrm{Adam} plays
  $\ell_{i\eqby?0}\!\!\step{\vec 0}\ell$ and \textrm{Eve} regains the
  control in $\ell(\vec v)$, which was also the configuration reached
  in one step in $\?M$, or \textrm{Adam} plays
  $\ell_{i\eqby?0}\!\!\step{\vec 0}\loc_{i}$ and \textrm{Eve}  regains the control in $\loc_{i}(\vec v)$ with
  $\vec v(i)=0$.  Using the actions
  $\loc_{i}\!\step{-\vec e_j}\loc_{i}$ for
  $j\neq i$, \textrm{Eve} can then reach $\loc_{i}(\vec 0)$ and move
  ...
  $\ell_{i\eqby?0}\!\!\step{\vec 0}\loc_{i}$, and in
  $\loc_{i}(\vec v)$ with $\vec v(i)>0$, there is no way to
  reach a configuration with an empty $i$th component, let alone to
  reach $\loc_\mathtt{halt}(\vec 0)$.  Thus, if $\?M$ halts, then \textrm{Eve}  has a winning strategy that mimics the unique play
...
:class: dropdown tip
  ...
  For all $0\leq d\leq p(n)$, we define $m_d\eqby{def}\sum_{0\leq e\leq
  d}2^{e}\cdot b_e$; thus $m-n+1=m_{p(n)}+1$.

  We define now the sets of
  locations
  \begin{align*}
    \Loc_\mathrm{Eve}\eqby{def}{\loc_0,\smiley\}
  ...
  p(n)\}\cup\{\loc_{1\leq?\leq 2^d}\mid 1\leq d\leq p(n)\}\;,\\
    \Loc_\mathrm{Adam}\eqby{def}{\loc_{(\gamma_1,\gamma_2,\gamma_3)}\mid\gamma_1,\gamma_2,\gamma_3\in\Gamma'\}\;.
  \end{align*}
  The intention behind the locations $\loc_{=j}\in\Loc_\mathrm{Eve} is
  that \textrm{Eve} can reach $\smiley(0)$ from a configuration $\loc_{=j}(c)$ if
  ...
  $\gamma_1,\gamma_2,\gamma_3\vdash\gamma$}\;,\\ \loc_{(\gamma_1,\gamma_2,\gamma_3)}&\step{0}\ellk_{\gamma_k}&\ellk_{\gamma_k}&\step{-k}\loc_{\gamma_k}&&\text{for
  $k\in\{1,2,3\}$}\;.
  ...
  of {numref}`11-fig:exp` until she reaches the initial left endmarker:}
   \loc_\triangleright\step{-m-2}\loc_\triangleright;,& \loc_\triangleright\step{-1}\loc_{=1}\;,& \loc_\triangleleft\step{-m-1}\loc_\triangleright;.
  ...
  action}
  \loc_\Box\step{-n-2}\loc_{1\leq?\leq m_{p(n)}+1}\;.
  \end{align*}

  Regarding the given initial credit variant, we add a new location
  $\ell_0$ controlled by \textrm{Eve} and let her choose her initial credit
  when starting from $\ell_0(0)$ by using the new actions
  $\ell_0\step{1}\ell_0$ and $\ell_0\step{0}\loc_0$.
...
${\uparrow}S\eqby{def}{x\in X\mid\exists y\in S\mathbin.y\leq x\}$.  A
...
${\downarrow}S\eqby{def}{x\in X\mid\exists y\in S\mathbin.x\leq y\}$, a
...
$V=\?Ltimes\+N^kcup\{\bot}$ is naturally equipped with a
...
i\leqk, i.e., if $\vec v\leq\vec v'$ for the componentwise
ordering.
...
$\textsf{col}e)\eqby{def}vcol(\textrm{In}e))$.  We
...
:class: dropdown tip
  ...
  \item[(zig \textrm{Eve}] if $v_1,v_2\in V_\mathrm{Eve}, $a\inA, and
    $\Deltav_1,a)=v'_1\neq\bot is defined, then
    $v'_2\eqby{def}dest(v_2,a)$ is such that $v'_2\geq v'_1$: indeed,
    $v'_1\neq\bot entails that $v_1$ is a configuration
    $\ell\vec v_1)$ and $v'_1=\ell(\vec v_1+\vec u)$ for the action
    $a=(\ellstep{\vec u}\ell)\inA, but then $v_2=\ell\vec v_2)$
    for some $\vec v_2\geq\vec v_1$ and
    $v'_2=\ell(\vec v_2+\vec u)\geq v'_1$;
  \item[(zig \textrm{Adam}] if $v_1,v_2\in V_\mathrm{Adam}, $a\inA, and
    $\Deltav_2,a)=v'_2$ is defined, then
    $v'_1\eqby{def}dest(v_1,a)\leq v'_2$: indeed, either $v'_2=\bot and
    then $v'_1=\bot, or $v'_2\neq\bot, thus
    $v_2=\ell\vec v_2)$, $v'_2=\ell(\vec v_2)$, and
    $a=(\ellstep{\vec 0}\ell)\inA (recall that the game is
    asymmetric), but then $v_1=\ell\vec v_1)$ for some
    $\vec v_1\leq\vec v_2$ and thus $v'_1=\ell(\vec v_1)\leq v'_2$.
  \end{description}
  The above conditions show that, if $\sigma{:}\,E^\ast\toA is a
...
$v_0\inW_\mathrm{Eve}and $v_0\leq v'_0$ imply $v_0'\inW_\mathrm{Eve}   {prf:ref}`11-lem:mono`
...
\cpageref{11-fig:cov}, $\ell(0,1)\inW_\mathrm{Eve}but $\ell(1,2)\inW_\mathrm{Adam}for
...
% set $\textsf{col}{-1}(d)$ for a colour $d\in C$ has finitely many minimal

% number

% \cref{11-pb:cov}, $\|\vec v\|$ is viable if $\ell\vec v)$ is the
...
vector game", $W_\mathrm{Eve}bigcup_{1\leq j\leq n}{\uparrow}\loc_j(\vec v_j)$
is a finite union of principal filters.  The set
$\mathsf{Pareto}\eqby{def}{\loc_1(\vec v_1),\dots,\loc_n(\vec v_n)\}$ is
...
````{prf:example} NEEDS LABEL Pareto limit
  ...
  configuration $\ell2,2)$, the Pareto limit is
  $\mathsf{Pareto}=\{\ell2,2),\ell(3,2),\loc_{2,1}(0,1),\loc_{\text-1,0}(3,2)\}$,
...
% $\mathsf{ParetoBound}\eqby{def}max_{\ell\vec v)\in\mathsf{Pareto}}\|\vec
...
% the two values always coincide in dimension $k1$.  We call a number
...
```
  ...
  $\mathsf{Pareto}\cap(\{\loc_0,\loc_1,\loc_f\}\times\+N^k=\{\loc_0(\vec
...
:class: dropdown tip
  ...
  action $\ellstep{\vec u}\ell\inA, if $\vec v'\geq\vec v$ and
  $m$~has a child $m'$ labelled by $\ell(\vec v+\vec u)$ in the
  self-covering tree, then either $m'$ is a leaf with "return
  node" $m''$ and we set $\delta(m,e)\eqby{def}m''$, or $m'$~is an
  internal node and we set $\delta(m,e)\eqby{def}m'$; in all the other
  cases, $\delta(m,e)\eqby{def}m_\bot$. 
...
```
  ...
  $\mathrm{lcol}\loc_0)\eqby{def}2$ and $\mathrm{lcol}\loc_1)\eqby{def}1$, note that \textrm{Eve}  must visit $\loc_0$ infinitely often in order to fulfil the parity
  requirements.  Starting from the initial
  configuration $\loc_0(\vec 0)$, any winning play of \textrm{Eve} begins
...
\begin{align*}
  U_0&\eqby{def}\uparrow}\loc_f(\vec v_f)\;,&
  U_{i+1}&\eqby{def}U_i\cup\mathrm{Pre}(U_i)\;.
\end{align*}
...
*  $v=\ell\vec v)\in\mathrm{Pre}(U_i)\capV_\mathrm{Eve}thanks to some
  $\ellstep{\vec u}\ell\inA such that
...
*  $v=\ell\vec v)\in\mathrm{Pre}(U_i)\capV_\mathrm{Adam}because for all
  $\ellstep{\vec 0}\ell\inA, $\ell(\vec v)\in U_i$; therefore
...
that $U_{i+1}\subseteq U_i$ and then $W_\mEveU_i$.  Thus the
...
sequence of sets
\begin{align*}
  B_0&\eqby{def}{\loc_f(\vec v_f)\}&B_{i+1}&\eqby{def}                                       B_i\cup\min\mathrm{Pre}({\uparrow}B_i)\;.
\end{align*}
