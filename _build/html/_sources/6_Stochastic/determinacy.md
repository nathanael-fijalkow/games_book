...
:class: dropdown tip
  ...
  Let $G = (VE,\delta,\textrm{Win}$ be a stochastic reachability
...
$$
  \mathfrak{F}(\nu)(v) = \left\{\begin{array}{l@{~~}l}
      ...
      \sum_{v' \in V \delta(v)(v') \cdot \nu(v') & \text{if} \in \vertices_{\text{Rand}}    \end{array}\right.
  $$
...
$$
    **lfp**\mathfrak{F}) (v) \le \sup_\sigma \inf_\tau
    \probm_{\sigma,\tau}^v(\mathtt{Reach}\textrm{Win}) \le \inf_\tau \sup_\sigma
    \probm_{\sigma,\tau}^v(\mathtt{Reach}\textrm{Win}) \enspace.
    $$
...
````
  ...
  \begin{proof}
    ...
    \begin{eqnarray*}
      ...
      \probm_{\sigma',\tau}^v(\mathtt{Reach}\textrm{Win}) & \le & \sup_\sigma
      \probm_{\sigma,\tau}^v(\mathtt{Reach}\textrm{Win}) \\
    ...
    \probm_{\sigma,\tau}^v(\mathtt{Reach}\textrm{Win})$ is a fixpoint of
 ...
 \begin{eqnarray*}
      \sup_\sigma \inf_\tau \probm_{\sigma,\tau}^v(\mathtt{Reach}\textrm{Win}) & = &
      \sup_\sigma \inf_\tau \sum_{v'} \sigma(v)(v')  \cdot
      \probm_{\sigma',\tau'}^{v'}(\mathtt{Reach}\textrm{Win}) \\
      & & \text{where}\ \sigma'\ \text{and}\ \tau'\ \text{are residual
        strategies after}\ (v,v') \\
      & = & \sup_\sigma \sum_{v'} \sigma(v)(v')  \cdot \inf_\tau \probm_{\sigma',\tau'}^{v'}(\mathtt{Reach}\textrm{Win}) \\
      ...
      & = & \mathfrak{F}(\probm_{\sigma,\tau}^{\bullet}(\mathtt{Reach}\textrm{Win}))(v)
    \end{eqnarray*}
    where $\probm_{\sigma,\tau}^{\bullet}(\mathtt{Reach}\textrm{Win}))$ is the
    ...
    \begin{eqnarray*}
      ...
      \cdot \probm_{\sigma',\tau'}^{v'}(\mathtt{Reach}\textrm{Win}) \\
      & & \text{where}\ \sigma'\ \text{and}\ \tau'\ \text{are residual
        strategies after}\ (v,v') \\
      ...
      & = & \min_{(v,v') \in E}  \sup_{\sigma'} \inf_{\tau\
        \text{s.t.}\ \tau(v) = v'} \probm_{\sigma',\tau'}^{v'}(\mathtt{Reach}\textrm{Win}) \\
      & & \text{since}\ \sigma\ \text{is not concerned with the first
        choice} \\
      & = & \mathfrak{F}(\probm_{\sigma,\tau}^{\bullet}(\mathtt{Reach}\textrm{Win}))(v)
    \end{eqnarray*}

    Finally, let  $v \in \vertices_{\text{Rand}}. 
    \begin{eqnarray*}
      \sup_\sigma \inf_\tau \probm_{\sigma,\tau}^v(\mathtt{Reach}\textrm{Win}) & = & \sum_{v'} \delta(v)(v') \cdot \sup_{\sigma'} \inf_{\tau'} \probm_{\sigma',\tau'}^{v'}(\mathtt{Reach}\textrm{Win})\\
            & & \text{where}\ \sigma'\ \text{and}\ \tau'\ \text{are residual
                strategies after}\ (v,v') \\
      & = & \mathfrak{F}(\probm_{\sigma,\tau}^{\bullet}(\mathtt{Reach}\textrm{Win}))(v)
    \end{eqnarray*}
...
$$
    \probm_{\sigma^*,\tau^*}^v(\mathtt{Reach}\textrm{Win}) = \sup_\sigma
    \probm_{\sigma,\tau^*}^v(\mathtt{Reach}\textrm{Win}) \enspace.
    $$

    Furthermore, $\probm_{\sigma^*,\tau^*}^\bullet(\mathtt{Reach}\textrm{Win})$ is
...
$$
      **lfp**\mathfrak{F}) \ge
      \probm_{\sigma^*,\tau^*}^\bullet(\mathtt{Reach}\textrm{Win}) = \sup_\sigma
      \probm_{\sigma,\tau^*}^\bullet(\mathtt{Reach}\textrm{Win}) \ge \inf_\tau
      \sup_\sigma \probm_{\sigma,\tau}^\bullet(\mathtt{Reach}\textrm{Win}) \enspace.
    $$
...
$$
   **lfp**\mathfrak{F}) =
   \sup_\sigma \inf_\tau 
    \probm_{\sigma,\tau}^\bullet(\mathtt{Reach}\textrm{Win}) =  \inf_\tau \sup_\sigma
   \probm_{\sigma,\tau}^\bullet(\mathtt{Reach}\textrm{Win}) = 
   \probm_{\sigma^*,\tau^*}^\bullet(\mathtt{Reach}\textrm{Win}) \enspace.
  $$
...
vertices $\ensuremath{v_{\textsc{win}}} and $\ensuremath{v_{\textsc{lose}}}, and for all strategies $\sigma$ and
$\tau$, for every vertex $v$,
$\probm_{\sigma,\tau}^{\mathcal{A}v}(\mathtt{Reach}\{\ensuremath{v_{\textsc{win}}}\ensuremath{v_{\textsc{lose}}}})) =
...
%   Let $\mathcal{A} be an arena with two distinguished sink vertices

%   $\mathtt{Reach}\textrm{Win}$ where $\textrm{Win}= \{\ensuremath{v_{\textsc{win}}}}$. The stochastic game $\mathcal{G}
%   $\sigma$ and $\tau$,
