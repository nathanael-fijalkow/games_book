...
in {cite}`BMS15,BFM15,LLTW14,ORS14`.% circumvent this problem: it~consists in **blaming** at each round% (even~if she reaches her goal) in case the infinite sequence of delays% semantics, \Eve~has a winning strategy in the game% avoid strategies that require arbitrary
...
are not fixed.A~different setting was developed in {cite}`CDLLR07`, with
...
and evaluate strategies for stochastic timed games {cite}`DJLMT15`.Another approach, relying on
