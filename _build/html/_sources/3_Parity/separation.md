...
%We define the objective $\Parity_{\mid n}$ over the set of colours $[1,d]$ as:%\Parity_{\mid n} = \bigcup \set{ \Paths(G,v) : %G \text{ parity graph with } n \text{ vertices and priorities in } [1,d] \\%\end{array}}.The definition of $(n,d)$-separating automata is illustrated in {numref}`3-fig:separation` and can be summarised
