...
then $S \setminus \textrm{Attr}_\mEveX)$ is a trapfor Eve in the subgame of $\Game$ induced by $V \setminus \textrm{Attr}_\mEveX)$.
*  Let $S$ be a trap for Eve in the game $\Game$ and $X$ a set of vertices such that $S \cap X = \emptyset$, 
then $S \subseteq V \setminus \textrm{Attr}_\mAdamX)$ and $S$ is a trap for Eve in the subgame of $\Game$ induced by $V \setminus \textrm{Attr}_\mAdamX)$.
