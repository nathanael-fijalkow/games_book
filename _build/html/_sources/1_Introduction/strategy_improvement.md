...
we refer to Chapter {ref}`3-chap:parity,4-chap:payoffs` for examples.%Assume that the initial strategy is $\sigma_0$ defined by %\sigma_0(v_1) = (v_1,v_1)$,%This strategy is losing from all vertices since it eventually ends up looping around $v_1$.%However $\sigma_0$ is clearly not optimal, contradicting the progress principle.%For this reason the initial strategy $\sigma_0$ must be carefully chosen.%and to define $\sigma_0$ to be the strategy choosing this option from every vertex.%%\centering%    \node[s-eve] (v0) at (0,0) {$\begin{array}{c} v_0 \\ 2 \end{array}$};%    \node[s-eve] (v2) at (4,0) {$\begin{array}{c} v_2 \\ 2 \end{array}$};%    \path[arrow]%      (v0) edge[bend left] (v1)%      (v1) edge[selfloop=90] (v1)%      (v2) edge[selfloop=0] (v2)%  \end{tikzpicture}%\label{1-fig:counter_example_strategy_improvement}
