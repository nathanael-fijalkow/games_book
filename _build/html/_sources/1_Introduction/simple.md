...
consistent with both strategies written $\pi^v_{\sigma,\tau}$. 
Note that the fact that it is infinite follows from our assumption that all
...
To be specific, the definition above is for $2$-player zero sum turn
...
We now state {ref}`Martin's determinacy theorem <1-thm:borel_determinacy>`.

```{admonition} Theorem (Martin's determinacy)
:name: 1-thm:borel_determinacy

Qualitative games with Borel objectives are determined.
```


The definition of Borel sets goes beyond the scope of this book. Suffice
to say that all objectives studied in this book are (very simple)
...
```{admonition} Computing the winning regions of a qualitative game
A qualitative game $\game$
...
```{admonition} Solving a quantitative game
A quantitative game $\game$, a vertex $v$, and a threshold $x \in \Qinfty$

Does Eve have a strategy ensuring $x$ from $v$?
```
...
```{admonition} The value problem for quantitative games
A quantitative game $\game$, a vertex $v$, and a threshold $x \in \Qinfty$

Is it true that $\Value^{\game}(v) \ge x$?
```
...
```{admonition} Computing the value for quantitative games
A quantitative game $\game$ and a vertex $v$
...
```{admonition} Computing the value function in quantitative games
A quantitative game $\game$

The value function $\Value^{\game} : V \to \Rinfty$
```
...
```{admonition} Constructing an optimal strategy in quantitative games
A quantitative game $\game$ and a vertex $v$

Construct an optimal strategy from $v$
```
