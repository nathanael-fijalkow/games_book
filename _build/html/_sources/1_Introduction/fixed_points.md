...
%It is said complete if all Cauchy sequences converge.%$d(\Op(x),\Op(y)) \le \lambda \cdot d(x,y)$.%\begin{theorem}[Banach's fixed point theorem]%Let $(X,d)$ be a complete metric space and $\Op : X \to X$ a contracting operator, then $\Op$ has a unique fixed point $x_*$.%
...
replacing inflationary by deflationary and preserves suprema by preserves infima.%which is monotonic and inflationary if $\Op$ is monotonic.
