...
We refer to {ref}`Section <2-sec:references>` for the role of parity objectives and how they emerged in automata theory as a subclass of Muller objectives.
...
in {ref}`Section <3-sec:value_iteration>` by considering the universal tree of size $n^h$.
...
through the notion of permissive strategies, and constructed a separating automaton\footnote{We note that the general framework of separating automata came later, introduced by Boja&#324;czyk and Czerwi&#324;ski {cite}`Bojanczyk&Czerwinski:2018`.} corresponding to the universal tree of size $n^h$.
...
The presentation using universal tree that we follow in {ref}`Section <3-sec:value_iteration>` and an almost matching lower bound on their sizes is due to Fijalkow {cite}`Fijalkow:2018`.
...
As discussed in {ref}`Section <3-sec:relationships>` the complexity of this algorithm is quasipolynomial and of the form $n^{O(\log(d))}$,
...
As we will see in {ref}`Chapter <4-chap:payoff>`, parity games can be reduced to mean payoff games,
...
as discussed in {ref}`Section <3-sec:relationships>` the notion of universal trees cannot be used to achieve this,
