...
```{figure} Fig/1-fig:arena_example.png
:name: 1-arena_example
:align: center
...
**RETURN**: A winning strategy for Eve from $v$
...
**OUTPUT**: Does Eve have a strategy ensuring $x$ from $v$?
...
**OUTPUT**: Is it true that $\Value^{\game}(v) \ge x$?
...
**OUTPUT**: The value function $\Value^{\game} : V \to \Rinfty$
...
**OUTPUT**: An optimal strategy from $v$
