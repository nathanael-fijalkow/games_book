...
$W_{>0}( \mathcal{M}, \mathtt{Reach}( c))$ is  NL-complete. Moreover, the set $W_{>0}( \mathcal{M}, \mathtt{Reach}( c))$ can be computed in linear time.
...
$W_{=1}( \mathcal{M}, \mathtt{Reach}( Win))=\{u,v\}$. However, $\{v\}\cup \text{Pre}_{=1}(\{v\})=\{v\}$, so 
...
$W_{=1}( \mathcal{M}, \mathtt{Safe}(Z))$ where $ Z =   W_{>0}( \mathcal{M}, \mathtt{Reach}( c))$. We might be tempted to think that this set already equals $  W_{=1}( \mathcal{M}, \mathtt{Reach}( c)) $, but this is not the case. To see this, consider an MDP with three states $ u,v,t $ and two actions $ a,b $ such that $ t $ is coloured by $  Win $, both actions self loop in $ v $ and $ t $, and $  \Delta(t \mid u,a) =  \Delta(v \mid u,a) = \frac{1}{2} $ while $  \Delta(u \mid u,b) = 1 $. Then $  W_{=1}( \mathcal{M}, \mathtt{Reach}( Win)) = \{t\} $ while at the same time $  W_{=1}( \mathcal{M}, \mathtt{Safe}(  W_{>0}( \mathcal{M}, \mathtt{Reach}( Win)))) = \{u,t\}$. However, iterating the computation works, as shown in the following theorem.
...
$W_{=1}( \mathcal{M}, \mathtt{Reach}( c))$ is  PTIME-complete.
