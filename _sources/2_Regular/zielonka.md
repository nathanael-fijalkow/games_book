...
There exists a deterministic parity automaton $\LAR_\mathcal{F} over the alphabet $C$ defining $\mathtt{Muller}\mathcal{F}$.
...
We define the automaton $\LAR_\mathcal{F}.
...
We now show that the automaton $\LAR_\mathcal{F} defines $\mathtt{Muller}\mathcal{F}$.
Let $\rho = c_0 c_1 \dots$ be an infinite word over the alphabet $C$. 
Let us consider the run of $\LAR_\mathcal{F} over $\rho$:
...
Thus $\rho$ is accepted by $\LAR_\mathcal{F} if and only if $\mathtt{Inf}\rho) \in \mathcal{F}, as desired.
...
%Let us consider the run $\ell_0 \ell_1 \dots$ of $\LAR_\mathcal{F} over $\rho$
...
$$
\mathcal{F}= \set{\set{2}, \set{3}, \set{4}, \set{1,2}, \set{1,3}, \set{1,3,4}, \set{2,3,4}, \set{1,2,3,4}}.
$$
...
Let $C = [1,d]$ be a set of colours and $\mathtt{Muller}\mathcal{F}$ a Muller objective.
There exists a deterministic parity automaton $\Zielonka_\mathcal{F} over the alphabet $C$ defining $\mathtt{Muller}\mathcal{F}$.
...
The set of states of $\Zielonka_\mathcal{F} is the set of branches of $T_\mathcal{F}.
...
because $\mathrm{supp}b,2) = \set{1,2,3}$.
...
We now show that the automaton $\Zielonka_\mathcal{F} defines $\mathtt{Muller}\mathcal{F}$.
Let $\rho = c_0 c_1 \dots$ be an infinite word over the alphabet $C$.
Let us consider the run of $\Zielonka_\mathcal{F} over $\rho$:
...
thus $\rho$ is accepted by $\Zielonka_\mathcal{F} if and only if $\mathtt{Inf}\rho) \in \mathcal{F}, as desired.
...
For instance in {numref}`2-fig:Zielonka_tree_example` the node labelled $\set{1,2,3}$ corresponds to $\mathtt{Muller}\mathcal{F})$ with
$\mathcal{F} = \set{\set{2}, \set{3}, \set{1,2}, \set{1,3}}$.
...
there are two cases:
\begin{itemize}
*  if $C \in \mathcal{F}, then $m_\mathcal{F} is the **sum** of $m_{\F_1},\dots,m_{\F_k}$;
*  if $C \notin \mathcal{F}, then $m_\mathcal{F} is the **maximum** of $m_{\F_1},\dots,m_{\F_k}$.
...
In the following result we consider **partial** colouring functions: $\textsf{col}: V \to C \cup \set{\emptyset}$,
...
Let $\mathcal{F}= \set{ \set{1,2} }$: the objective $\mathtt{Muller}\mathcal{F}$ requires that both $1$ and $2$ are seen infinitely many times.
The Zielonka tree $T_\mathcal{F} has three nodes $\set{1,2}, \set{1}$, and $\set{2}$, and $m_\mathcal{F}= 2$.
...
where Eve has a winning strategy for $\mathtt{Muller}\mathcal{F}$ with $\mathcal{F}= \set{ \set{1,2} }$ using two memory states
...
and define $\Game_i$ be the subgame of $\Game$ induced by $V \setminus \textrm{Attr}_\mEveC \setminus C_i)$
with objective $\mathtt{Muller}\F_i)$.
...
$$
m_{2^C \setminus \mathcal{F} = \max_{i \in [1,k]} m_{2^{C_i} \setminus \F_i}.
$$
...
For each $i \in [1,k]$, let $\sigma_i$ be an attractor strategy ensuring to reach $C_i$ from $\textrm{Attr}_\mEveC_i)$,
and consider a winning strategy for Eve from $V \setminus \textrm{Attr}_\mEveC_i)$ in $\Game_i$, it induces a strategy $\sigma'_i$ in $\Game$.
...
The strategy $\sigma$ with memory $i$ simulates $\sigma_i$ from $\textrm{Attr}_\mEveC_i)$ and $\sigma'_i$ from $V \setminus \textrm{Attr}_\mEveC_i)$,
...
or eventually remains in $V \setminus \textrm{Attr}_\mEveC_i)$ and is eventually consistent with $\sigma'_i$.
...
Since $V \setminus \textrm{Attr}_\mEveC_i)$ is a trap for Eve, this implies that $\tau_i$ is a winning strategy in $\Game$.
Let $\tau_a$ denote an attractor strategy from $\textrm{Attr}_\mAdamW_\mathrm{Adam}Game_i)) \setminus W_\mathrm{Adam}Game_i)$.
...
We construct a strategy $\tau$ in $\Game$ as the (disjoint) union of the strategy $\tau_a$ on $\textrm{Attr}_\mAdamW_\mathrm{Adam}Game_i)) \setminus W_\mathrm{Adam}Game_i)$,
...
Since $\tau$ is a disjoint union of strategies the memory can be reused so we can implement $\tau$ using $\max_{i \in [1,k]} m_{2^{C_i} \setminus \F_i}$ memory states, corresponding to the definition of $m_{2^C \setminus \mathcal{F}$.
...
We let $\F_1,\dots,\F_k$ be the corresponding induced Muller objectives,
and define $\Game_i$ be the subgame of $\Game$ induced by $V \setminus \textrm{Attr}_\mAdamC \setminus C_i)$ with objective $\mathtt{Muller}\F_i)$.
