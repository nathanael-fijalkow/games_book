...
````{prf:corollary} One-dimensional vector games are in \textrm{EXPSPACE}:label: 11-cor:dim1
...
$\?V=(\?LA\Loc_\mathrm{Eve}\Loc_\mathrm{Adam}1)$ is called a countdown
system if $Asubseteq\?Ltimes\+Z_{<0}\times\?L, that is, if
for all $(\ellstep z\ell)\inA, $z<0$.  We consider the games
...
:class: dropdown tip
  ...
  system" $\?V=(\?LA\Loc_\mathrm{Eve}\Loc_\mathrm{Adam}1)$, an initial
  location $\loc_0\in\?L, an initial credit $n_0\in\+N$, and a
  target configuration $\loc_f(n_f)\in\?Ltimes\+N$.  Because every
  ...
  positive.  Let $m\eqby{def}2^{p(n)}\geq n$.  The computation of $\?M$
...
```
  ...
  We now set up a two-player game where \textrm{Eve} wants to prove that the
  input $w$ is accepted.  Let
  $\Gamma'\eqby{def}\{\triangleright\triangleleft}\cup\Gamma\cup(Q\times\Gamma)$. Rather
  than exhibiting the full computation from {numref}`11-fig:exp`, the
  game will be played over positions $(i,j,\gamma_{i,j})$ where
  $0<i\leq m$, $0\leq j\leq m+1$, and $\gamma_{i,j}\in\Gamma'$.  \textrm{Eve}  wants to show that, in the computation of $\?M$ over $w$ as depicted
  ...
  configuration.  Thus, if $i>1$ and $0<j<m+1$, \textrm{Eve} provides a triple
  ...
  and \textrm{Adam} chooses $j'\in\{j-1,j,j+1\}$ and returns the control
  to \textrm{Eve} in position $(i-1,j',\gamma_{i-1,j'})$.  Regarding the
  boundary cases where $i=0$ or $j=0$ or $j=m+1$, \textrm{Eve} wins
  immediately if $j=0$ and $\gamma={\triangleright$, or if $j=m+1$ and
  $\gamma={\triangleleft$, or if $i=0$ and $0<j\leq n$ and $\gamma=a_j$, or if
  $i=0$ and $n<j\leq m$ and $\gamma={\Box$, and otherwise \textrm{Adam} wins
  immediately.  The game starts in a position
  $(t,j,(q_\mathrm{final},a))$ for some $0<t\leq m$, $0< j\leq m$,
  and $a\in\Gamma$ of \textrm{Eve}s choosing.  It should be clear that \textrm{Eve}  has a winning strategy in this game if and only if $w$ is accepted
  by $\?M$.

  We now implement the previous game as a coverability game over a
  countdown system $\?V\eqby{def}\?LA\Loc_\mathrm{Eve}\Loc_\mathrm{Adam}1)$.
  ...
  represented by configuration $\loc_{\triangleright(2)$, the first input
  $(q_0,a_1)$ at position $(1,1)$ by $\loc_{(q_0,a_1)}(3)$, and the
  endmarker $\triangleleft at position $(m,m+1)$ by
  $\loc_{\triangleleft(m\cdot(m+2)+1)$. The game starts from the initial
  configuration $\loc_0(n_0)$ where $n_0\eqby{def}m\cdot(m+2)+1$ and the
  target location is $\smiley$.

  We define for this the sets of locations
  \begin{align*}
    \Loc_\mathrm{Eve}\eqby{def}{\loc_0,\smiley,\frownie\}
  ...
  \end{align*}
  The intention behind the locations $\loc_{=j}\in\Loc_\mathrm{Adam} is
  that \textrm{Eve} can reach $\smiley$ from a configuration $\loc_{=j}(c)$ if
  ...
  \intertext{Similarly, \textrm{Eve} should be able to reach $\smiley$ from
  ...
  \end{align*}
  Note this last action also ensures that \textrm{Eve} can reach the
  ...
  action}
  \loc_\Box\step{-n-2}\loc_{1\leq?\leq m-n+1}\;.&&&&&\qedhere\hspace*{-1.5em}
  \end{align*}
...
:class: dropdown tip
   ...
   For the upper bound, consider an instance, i.e., a "countdown
   system" $\?V=(\?LA\Loc_\mathrm{Eve}\Loc_\mathrm{Adam}1)$, an initial
   location $\loc_0$, and a target configuration $\loc_f\in\?L.  We
   reduce this to an instance of configuration reachability with
   given initial credit in a one-dimensional vector system by
   adding a new location $\ell_0$ controlled by~\textrm{Eve} with actions
   $\ell_0\step{1}\ell_0$ and $\ell_0\step 0\loc_0$, and asking
   whether \textrm{Eve} has a winning strategy starting from $\ell_0(0)$ in
...
\AP\decpb[zero reachability with given initial credit]
  {A countdown system $\?V=(\?LT,\Loc_\mathrm{Eve}\Loc_\mathrm{Adam}1)$, an
  initial location $\loc_0\in\?L, and an initial credit
  $n_0\in\+N$.}
  {Does \textrm{Eve} have a strategy to reach a configuration $\ell0)$ for
  some $\ellin\?L?\\That is, does she win the zero
  reachability\index{zero reachability|see{countdown game}}
  game $(\?A_\+N(\?V),\textsf{col}\mathtt{Reach}$ from $\loc_0(n_0)$, where
  $\textsf{col}e)=\textrm{Win} if and only if $\textrm{In}e)=\ell0)$ for some $\ellin\?L?}
...
:class: dropdown tip
  ...
  the only way for \textrm{Eve} to bring the counter to zero in this new game
...
:class: dropdown tip
  ...
  parity@parity vector game is also \textrm{EXPSPACE}hard.  Finally, we can
  ...
  new initial location $\ell_0$ controlled by \textrm{Eve} with actions
  $\ell_0\step 1\ell_0$ and $\ell_0\step 0\loc_0$ and play the game
  starting from $\ell_0(0)$.
  ...
  given initial credit with an initial incrementing loop $\ell_0\step
  1\ell_0$ does not work, because \textrm{Eve} would have a trivial winning
