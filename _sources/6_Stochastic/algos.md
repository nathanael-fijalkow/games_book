...
````{prf:definition} Binary stochastic arenas
...
````{prf:proposition} Reduction from stochastic games to simple stochastic games
...
````{prf:definition} Stopping simple stochastic games
...
We define the deterministic attractor operator $\textrm{DetAtt}$ as
...
$$
 \textrm{DetAtt}(X) = \lim_{i \to +\infty} X_i = \bigcup_{i =0}^{+\infty} X_i
$$
...
W_\pi^{k+1}  = \{ v_{win}\} \\
W_\pi^i =
 \textrm{DetAtt}(\{ \pi_i,\ldots, \pi_k, v_{win}\}) \setminus \bigcup_{j=i+1}^{k+1}
...
````{prf:theorem} Correctness of the strategy improvement algorithm
...
That is, the order given by $\pi$ coincides with the preference of $\textrm{Eve}$.
...
````{prf:lemma} Correctness of live and self-consistent permutations
:label: stoch:lemma2
...
deduce that $v \in  \textrm{DetAtt}(w) \subseteq
 \textrm{DetAtt}(\{ \pi_j,\ldots, \pi_k, v_{win}\})$, which is not the case,
...
````{prf:corollary} Self-consistent implies solution of Bellman's equations
...
`stopping' MDP when $\textrm{Eve}$ plays according to $\sigma_\pi$: 
$\textrm{Adam}$ will not be able to prevent the game converging to $v_{lose}$ and $v_{win}$.

````{prf:lemma} Live permutations imply stopping MDPs
...
````{prf:lemma} Live and self-consistent implies lower bound on values
...
$\textrm{val}_\pi(v) \le   \textrm{val}^*(v)$.

````

````{prf:corollary} Live and self-consistent imply optimal
...
````{prf:lemma} Live and value non-decreasing imply self-consistent
...
$Y = V \setminus  \textrm{DetAtt}(X)$. Then either $Y = \{ v_{lose}\}$, or there
is a random vertex $v \in Y$ such that $\textrm{val}^*(v) = \max\{  \textrm{val}^*(w)
\mid w \in Y\}$ and $\delta(v)\Big( \textrm{DetAtt}(X)\Big)>0$.
...
$\delta(v)\Big( \textrm{DetAtt}(X)\Big)>0$. We will show that $Z =
...
$\textrm{DetAtt}(X)$: by definition, $\tau(Y) \subseteq Y$. Also, one can
...
every $w'$ such that $\delta(v')(w')>0$, $\textrm{val}^*(w') =
 ...
 \textrm{DetAtt}(X)$, hence $w' \in Z$.  Let $v' \in Z \cap   V_\mathrm{Eve}$. If
there is $w' \notin Z$ such that $(v',w') \in E$, then it must be
the case that $w' \in Y \setminus Z$: indeed, it cannot be the case
that $w' \in  \textrm{DetAtt}(X)$, otherwise $v'$ would also be in
$\textrm{DetAtt}(X)$.
...
We will define a permutation $\pi$ inductively, by repeatedly using {prf:ref}`6-lem:structure`.
For every $i =k, \ldots ,1$ we define $\pi_i$ by applying {prf:ref}`6-lem:structure` to 
$X = \{ \pi_{i+1},\ldots, \pi_k, v_{win}\}$.
...
To obtain the polynomial-time complexity claimed in {prf:ref}`6-thm:corr-strat-improv`, 
we realize that once a permutation $\pi$ is fixed, computing the sets $W_\pi^i$ can be done
...
For the third property, we realize that $\pi'$-regions in $\mathcal{G}[\sigma_\pi]$ are included in $\pi'$-regions in $\mathcal{G}$, which immediately implies the result.
