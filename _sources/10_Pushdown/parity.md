...
$t\! t$ (Proposition \cref\{prop:par_dir_dep_paritexp} will
...
Assume now by contradiction that $\widehat{\sigma}$ is not winning and consider a loosing play ```{math}\n
\begin{split}
 \widehat{ \pi} = & (\bot,R_0)(\gamma_0,R_0,\sigma_0,S_0)(\gamma_0,R_0,\sigma_0,S_0,(\sigma^1_\alpha,S^1_\alpha)_{\alpha\in\Gamma}))(\gamma_1,R_1,\sigma_1,S_1)\\ &\quad (\gamma_1,R_1,\sigma_1,S_1,(\sigma^2_\alpha,S^2_\alpha)_{\alpha\in\Gamma}))(\gamma_2,R_2,\sigma_2,S_2)\cdots
\end{split}
\\n```
...
Now, for every configuration $(q,s)$ with $q\in Q_\mathrm{Eve}$, we let $(\gamma,R,\sigma_\gamma,S_\gamma)$ be the last vertex in $\widehat{ \pi}_s$ and if $q\in R$ we let $\sigma((q,s)) = \sigma_\gamma(q)$ and otherwise we pick an arbitrary transition for $\sigma((q,s))$ as $(q,s)$ will be a loosing position for  \textrm{Eve} (see \Cref{10-proposition:R-winning-states} below).
...
Consider an infinite play $\pi=v_0v_1\cdots$ starting from some winning position for  \textrm{Eve}. Then by \Cref{10-proposition:stays-in-winning-region} we know that the play stays in the winning region. 
...
Now let $v_{i_j}=(q_j,s_j)$, $v_{i_{j+1}-1}=(p_j,s_j)$ and let $c_j$ be the largest colour visited in $v_{i_j}\cdots v_{i_{j+1}-1}$. Then, as we assume that $\pi$ is loosing one has $\limsup(c_i)_{i\geq 0}$ is odd. Moreover, if one lets ```{math}\n
\begin{split}
 \widehat{ \pi} = & (\bot,R_0)(\gamma_0,R_0,\sigma_0,S_0)(\gamma_0,R_0,\sigma_0,S_0,(\sigma^1_\alpha,S^1_\alpha)_{\alpha\in\Gamma}))(\gamma_1,R_1,\sigma_1,S_1)\\ &\quad (\gamma_1,R_1,\sigma_1,S_1,(\sigma^2_\alpha,S^2_\alpha)_{\alpha\in\Gamma}))(\gamma_2,R_2,\sigma_2,S_2)\cdots
\end{split}
\\n```
