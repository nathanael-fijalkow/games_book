...
vector $\overrightarrow{S}$ claimed by  Eve; if later on, a pop transition is simulated, the play
...
simulating a finite play $\pi$ in $\mathcal{G}$ such that:
    ...
    *   Eve claims that she has a strategy to continue $\pi$ in such
...
A vertex $(p,\alpha, \overrightarrow{R},c)$ is controlled by  Eve if
and only if $p\in Q_\mathrm{Eve}$.

*  The vertices $t\! t$ and $f\!\! f$ are here to ensure
  that the vectors $\overrightarrow{R}$ encoded in the main vertices are correct. They are both controlled by  Eve and are sink vertices with a self loop with colour $0$ for $t\! t$ and $1$ for $f\!\! f$.
...
To simulate a stair,  Adam goes, through an edge coloured by $\textsf{col}(q)$, to the vertex
...
1. [(i)] A configuration $(p_{in},\bot)$ is winning for  Eve in $\mathcal{G}$
if and only if
...
Assume that the configuration $(p_{in},\bot)$ is winning for  Eve in $\mathcal{G}$,
and let $\sigma$ be a corresponding winning strategy for her.

Using $\sigma$, we define a strategy $\widetilde{\sigma}$ for  Eve in
...
*  If $\sigma( \pi)=(r,pop)$, then  Eve goes to
$t\! t$ (Proposition {prf:ref}`prop:par_dir_dep_paritexp` will
  prove that this move is always possible).

*  If $\sigma( \pi)=(q,push(\beta))$, then  Eve goes to
$(p,\alpha, \overrightarrow{R},c,q,\beta)$.

In this last case, or in the case where $p\in Q_\mathrm{Adam}$ and  Adam goes
...
meantime. In order to define $\overrightarrow{S}$,  Eve considers the set of
...
\begin{split}
S_i=&\{r\mid \exists\ \pi\cdot(q,s\alpha\beta)v_0\cdots v_k(r,s\alpha)\cdots\text{ play in }  \mathcal{G}  \text{ where  Eve respects } \sigma \text{ and s.t. }\\ &  \mathrm{sh}(v_j)>|\sigma\alpha|,\ \forall j=0,\dots,k  \text{, and }\max(\{  \textsf{col}(v_j)\mid j=0,\dots,k\}\cup\{  \textsf{col}(q)\})=i\}
\end{split}

$$

Finally, we set $\overrightarrow{S}=(S_0,\dots,S_d)$ and  Eve moves to
...
Therefore, with any finite play $\widetilde{ \pi}$ in $\widetilde{ \mathcal{G}}$ in which  Eve
respects her strategy $\widetilde{\sigma}$, is associated a finite play $\pi$ in
$\mathcal{G}$. An immediate induction shows that  Eve respects $\sigma$ in
...
and where  Eve respects $\widetilde{\sigma}$. Let $\pi$ be the play associated with $\widetilde{ \pi}$
...
3.  Assume that $\pi$ is extended, that  Eve keeps respecting
...
$t\! t$. Moreover, one can deduce that, if  Eve respects $\widetilde{\sigma}$, $f\!\! f$ is never reached.
...
$(p_{in},\bot,(\emptyset,\dots,\emptyset),$ $\textsf{col}(p_{in)})$,
and where  Eve respects $\widetilde{\sigma}$. Let $\pi$ be the associated
...
$(p_{in},\bot,(\emptyset,\dots,\emptyset),$ $\textsf{col}(p_{in)})$
where  Eve respects $\widetilde{\sigma}$, the sequence of visited colours in $\widetilde{ \pi}$ is
...
is winning for  Eve, it follows that $\widetilde{ \pi}$ is also winning for
her.

(10-subsec:strategy-pushdown)=
### Proof of the Converse Implication of {prf:ref}`10-thm:games`

Note that in order to prove the converse implication of {prf:ref}`10-thm:games` one could follow the direct implication and consider the point of view of  Adam. Nevertheless the proof we give here starts from a winning strategy for  Eve in $\widetilde{ \mathcal{G}}$ and deduces a strategy for her in $\mathcal{G}$: this induces a more involved proof but has the advantage to lead to an effective construction of a winning strategy for  Eve in $\mathcal{G}$ if one has an effective strategy for her in $\widetilde{ \mathcal{G}}$

Assume now that  Eve has a winning strategy $\widetilde{\sigma}$ in $\widetilde{ \mathcal{G}}$
from $(p_{\mathit in},\bot,(\emptyset,\dots,\emptyset),  \textsf{col}(p_{in)})$.
Using $\widetilde{\sigma}$, we build a strategy $\sigma$ for  Eve in
...
in},\bot,(\emptyset,\dots,\emptyset),  \textsf{col}(p_{in)})$ and where
 Eve respects her winning strategy $\widetilde{\sigma}$. Moreover, for any
play $\pi$ where  Eve respects $\sigma$, we will always have that
...
describe how  Eve plays if $p\in Q_\mathrm{Eve}$, and then we explain how
...
If it is a move to $t\! t$,  Eve plays a transition
...
$(p,\alpha, \overrightarrow{R},c,q,\beta)$, then  Eve applies
...
Intuitively, $\overrightarrow{S}$ describes which states  Eve can force a play to
...
Let $\pi$ be a finite play in $\mathcal{G}$, where  Eve respects
...
$(p,\alpha, \overrightarrow{R},c)$ is winning for  Eve in
...
for  Eve) to the (losing) vertex $f\!\! f$. Hence we
...
$(p_{in},\bot)$ and where  Eve respects $\sigma$. Let $( \pi_i)_{i\geq 0}$ be its rounds factorisation. Let
...
always exists, is infinite and corresponds to a play won by  Eve in $\widetilde{ \mathcal{G}}$.
...
Therefore $\pi$ is won by  Eve. Using {prf:ref}`10-lemme:toto_paritexp` and {prf:ref}`10-prop:trans_cond`,
...
1. [(i)] One can construct in time $\mathcal{O}(m^d 2^{nd^2})$ a deterministic finite state automaton with $2^n$ states recognising the winning region of  Eve in $\mathcal{G}$.
2. [(ii)] One can decide in time $\mathcal{O}(m^d 2^{nd^2}+|s|)$, for any configuration $(p,\bot s)$, whether it is winning for  Eve in $\mathcal{G}$.
...
Let $\mathcal{G}$ be a reachability pushdown game. Then the following problem is hard for  EXP: decide whether $(p,\bot)$ is winning for  Eve in $\mathcal{G}$.
...
:label: 10-thm:hgame
 ...
 Eve has a finite memory winning strategy in $\widehat{ \mathcal{G}}$ from $(\bot,W(\emptyset,\bot))$.
...
As the game $\widehat{ \mathcal{G}}$ is played on a finite arena and equipped with an $\omega$-regular objective, it suffices to prove that  Eve has a winning strategy from $(\bot,W(\emptyset,\bot))$.

Consider a **positional** strategy $\sigma$ for  Eve in $\mathcal{G}$ that is winning on the whole winning region $W_\mathrm{Eve}$. Note that existence of positional winning strategies is ensure because $\mathcal{G}$ is a parity game.
...
\begin{split}
S^s_\sigma=& \{(p,c,q) \mid \exists  \pi=v_0\cdots v_k \text{ with $k>0$, $v_0=(p,s)$, $v_k=(q,s)$, $\mathrm{sh}(v_i)\geq  \mathrm{sh}(v_0)$ }\\  & \text{for all $0\leq i\leq k$, and such that  Eve respects $\sigma$ in $\pi$ and $c$ is the largest}\\ & \text{colour visited in $\pi$}\}
\end{split}
...
Using $\sigma$ we define a strategy $\widehat{\sigma}$ for  Eve in $\widehat{ \mathcal{G}}$ and we later argue that it is winning for her from $(\bot,W(\emptyset,\bot))$. 
...
$$
 ...
 and let $s_{ \widehat{ \pi}} = \gamma_0\cdots\gamma_k$. Then,  Eve goes to $(\gamma_k,R_k,\sigma_k,S_k,(\sigma^{k+1}_\alpha,S^{k+1}_\alpha)_{\alpha\in\Gamma}))$ with $\sigma^{k+1}_{\alpha}(r) = \sigma((r,s_{ \widehat{ \pi}}\alpha))$ for every $r$ such that $(r,s_{ \widehat{ \pi}}\alpha)\in W_\mathrm{Eve}$ and $S^{k+1}_\alpha = S_\sigma^{ \widehat{ \pi}\alpha}$. 
...
Then it is easily seen by definition that $\pi$ is loosing (because $\widehat{ \pi}$ is) while  Eve respects her winning strategy $\sigma$, which leads a contradiction and concludes the proof.

````

Following {prf:ref}`10-thm:hgame`, fix a finite memory winning strategy $\widehat{\sigma}$ for  Eve in $\widehat{ \mathcal{G}}$ from $(\bot,W(\emptyset,\bot))$. Using $\widehat{\sigma}$ we define a positional strategy $\sigma$ for  Eve in $\mathcal{G}$. 

First, we inductively associate, with any word $s\in \bot \Gamma^*$, a finite play $\widehat{ \pi}_s$ in $\widehat{ \mathcal{G}}$ where  Eve respects her strategy $\widehat{\sigma}$: 
...
Now, for every configuration $(q,s)$ with $q\in Q_\mathrm{Eve}$, we let $(\gamma,R,\sigma_\gamma,S_\gamma)$ be the last vertex in $\widehat{ \pi}_s$ and if $q\in R$ we let $\sigma((q,s)) = \sigma_\gamma(q)$ and otherwise we pick an arbitrary transition for $\sigma((q,s))$ as $(q,s)$ will be a loosing position for  Eve (see {prf:ref}`10-proposition:R-winning-states` below).
...
The following is  consequence of {prf:ref}`10-proposition:R-winning-states` and of the requirement that in a vertex $(\gamma,R,\sigma_\gamma,S)$  Eve should only propose $(W(R,\alpha),\alpha)$-local strategies that pops in $R$.

````{prf:proposition} NEEDS TITLE 10-proposition:stays-in-winning-region
:label: 10-proposition:stays-in-winning-region

Let $\pi$ be an infinite play in $\mathcal{G}$ starting from some winning position for  Eve and where  Eve respects strategy $\sigma$. Then any vertex visited in $\pi$ is a winning position for  Eve.
...
The positional strategy $\sigma$ is winning for  Eve on the whole winning region in $\mathcal{G}$.
...
Consider an infinite play $\pi=v_0v_1\cdots$ starting from some winning position for  Eve. Then by {prf:ref}`10-proposition:stays-in-winning-region` we know that the play stays in the winning region. 
