...
\begin{svgraybox}
\textbf{INPUT}: A "qualitative game" $\game$ and a vertex $v$

\textbf{QUESTION}: Does Eve win $\game$ from $v$?
\end{svgraybox}
...
\begin{svgraybox}
\textbf{INPUT}: A "qualitative game" $\game$

\textbf{RETURN}: $\WE(\game)$ and $\WA(\game)$
\end{svgraybox}
...
\textbf{RETURN}: A winning strategy for Eve from $v$
\end{svgraybox}
...
\vskip1em
\textbf{INPUT}: A "quantitative game" $\game$, a vertex $v$, and a threshold $x \in \Qinfty$

\textbf{OUTPUT}: Does Eve have a strategy ensuring $x$ from $v$?
\end{svgraybox}
...
\textbf{OUTPUT}: Is it true that $\Value^{\game}(v) \ge x$?
\end{svgraybox}
...
\begin{svgraybox}
\textbf{INPUT}: A "quantitative game" $\game$ and a vertex $v$

\textbf{OUTPUT}: $\Value^{\game}(v)$
\end{svgraybox}
...
\begin{svgraybox}
\textbf{INPUT}: A "quantitative game" $\game$

\textbf{OUTPUT}: The value function $\Value^{\game} : V \to \Rinfty$
\end{svgraybox}
...
\textbf{OUTPUT}: An optimal strategy from $v$
\end{svgraybox}
