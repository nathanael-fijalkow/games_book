...
weights $c( \pi_i)$ are bounded). The coefficient $1-\lambda$ before
...
````{prf:theorem} Positional determinacy
...
:class: dropdown tip
  ...
  payoff game is the **unique** solution of the fixed point
...
For the discounted payoff game of {numref}`4-fig:MP`, the contracting
operator is:
...
:class: dropdown tip
  ...
  $x_v \leq (1-\lambda)\sum_{i=0}^{n-1}\lambda^i c( \pi_i) +
  \lambda^nx_{v_n}$. Letting $n$ go to $+\infty$, we get that
  $x_v\leq  \mathtt{DiscountedPayoff}_\lambda( \pi)$. Since this holds for all
...
````{prf:theorem} Strategy improvement
...
of {eq}`4-eq:F-contraction` is to compute the sequence
...
````{prf:lemma} Number of steps of value iteration
...
````{prf:theorem} Correctness and complexity of value iteration
...
````{prf:theorem} Optimal strategies
:label: 4-thm:DP-strategies
  ...
  found in
  $O\big((n^3b\log_2b + \log_2W)\allowbreak
  \log(m/n) m\big)$ time.
...
````{prf:theorem} Discounted payoff approximation
...
````{prf:theorem} Direct value iteration
