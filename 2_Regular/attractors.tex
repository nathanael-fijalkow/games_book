...
and it is computed by the following sequence: we let $\AttrE^0(\Win) = \Win$ and\footnote{For technical convenience we shift the sequence by one, ignoring the first term which is the empty set} 
\[
