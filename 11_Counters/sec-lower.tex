...
following lower bound matching \cref{12-exist-easy}, already with a
unary encoding.

\begin{theorem}\label{12-exist-hard}
...
\end{theorem}
\begin{proof}
  By \cref{12-nonterm2parity}, it suffices to show hardness for
  ...
  \begin{scope}
    ...
    credits.  By \cref{12-counterless}, he has a "counterless" winning
  ...
  % "non-termination game" with "existential initial credit" if and only
  % if there is no such partition.  By \cref{12-nonterm2parity}, this
  ...
  % \end{tikzpicture}
  % \caption{\label{12-fig-part} The "vector system" built from a
  ...
  %               j\}\;;
  % \end{align*}
  % see \cref{12-fig-part} for a depiction of~$\?V$.  We use~$\loc_0$ as
...
Note that \cref{12-exist-hard} does not apply to fixed dimensions
$\dd\geq 2$.  We know by \cref{12-exist-pseudop} that those games can
be solved in pseudo-polynomial time if the number of priorities is
fixed, and by \cref{12-exist-easy} that they are in \coNP.

\subsubsection{Given Initial Credit}
With "given initial credit", we have a lower bound matching the
\kEXP[2] upper bound of \cref{12-avag-easy}, already with a unary
...
\begin{theorem}\label{12-avag-hard}
...
\begin{proof}
  ...
  %%%%%\begin{scope}
    ...
    the gadget of \cref{12-fig-meta-incr} below, where
    ...
    \end{tikzpicture}
    \caption{\label{12-fig-meta-incr} A naive implementation of the
  ...
  where we would simply go to~$\loc'$ in \cref{12-fig-meta-incr} at
  ...
  by \cref{12-cov2parity}, this will also entail the \kEXP[2]-hardness
  ...
  actions depicted in \cref{12-fig-lipton}.  Those maintain the
  ...
  \begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{\label{12-fig-lipton}Schema of the reduction to
      "coverability" in the proof of \cref{12-avag-hard}.}
  \end{figure}
  ...
  by~one at every step; see \cref{12-fig-lipton-nonterm}.  We also add
  ...
  \begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{\label{12-fig-lipton-nonterm}Schema of the reduction to
      "non-termination" in the proof of \cref{12-avag-hard}.}
  \end{figure}
...
The proof of \cref{12-avag-hard} relies crucially on the fact that the
...
order to match the \EXP\ upper bound of \cref{12-avag-easy} in fixed
dimension.

\begin{theorem}\label{12-avag-two}
  "Coverability", "non-termination", and "parity@parity vector game"
  "asymmetric" "vector games" with "given initial credit" are
...
\begin{proof}
  ...
  credit", which are \EXP-complete by \cref{12-countdown-given}.
  ...
  of \cref{12-countdown-given}, we can indeed assume that the target
  ...
  in~$\?V$.  The translation is depicted in \cref{12-fig-dim2}.
  ...
  \begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{\label{12-fig-dim2}Schema of the reduction in the proof
    of \cref{12-avag-two}.}
  \end{figure}
  ...
  from \cref{12-cov2parity,12-nonterm2parity}.
\end{proof}
