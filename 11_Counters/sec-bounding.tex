...
of \cref{12-pb-bounding} is obtained by quantifying~$\vec v_0$
existentially in the question.
\decpb["bounding game" with "given initial credit"]%
{\label{12-pb-bounding} A "vector system"
...
\begin{lemma}\label{12-parity2bounding}
...
\begin{proof}
  ...
  $\dd'\eqdef\dd+d$ as described in \cref{12-fig-bounding}, where the
  ...
  \begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{\label{12-fig-bounding}Schema of the reduction to
      "bounding games" in the proof of \cref{12-parity2bounding}.}
  \end{figure}
  ...
  by \cref{12-fact-mono} she can simply ignore the new decrement
  ...
  %% \begin{claim}\label{12-cl-capped}
  ...
  v_0)\in\mathsf{Pareto}(\game)$.  As seen in \cref{12-fact-finmem},
  ...
  \begin{claim}\label{12-cl-parity2bounding} If $\loc_0(\vec
    v)\in\WE(\game)$, then
  ...
  proof of \cref{12-fact-finmem}.  This is a winning strategy for \Eve
  in $\game$ starting from $\loc_0(\vec v_0)$, and
  by \cref{12-fact-mono}, it is also winning
...
The bound~$B$ defined in~\cref{12-b} in the previous proof is not
...
\cref{12-cov2parity}, i.e., where the objective is to reach some
...
for details.
\begin{theorem}\label{12-th-bounding}
...
\Cref{12-th-bounding} also yields a way of handling "bounding games"
