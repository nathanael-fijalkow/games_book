...
%%
## Lower Bound
%Here we show that even for reachability condition the problem is hard for exponential time. We give the proof of Walukiewicz that reduces from the acceptance problem for linear-space alternating Turing machine.% Hardness for PSPACE when using a counter instead of a general stack (proof ?)%
## Alternative Approaches
%Connection the full binary tree. Again regularity of the winning region plus the existence of regular positional strategies with Rabin's lemma. %%%%%\section{Beyond Parity Pushdown Games}%
