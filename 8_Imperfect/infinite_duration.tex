...
%The following random variables indexed by $n\in \N$ are useful.
%\begin{itemize}
%\item %$V_n$ the $n$-th vertex visited by the play,
%\item %$A_n,B_n$ the $n$-th actions chosen by Eve and Adam respectively,
%\item %$S_n,T_n$ the $n$-signals received by Eve and Adam respectively,
%\item %$C_n$ the $n$-th color seen during the play.
%\end{itemize}
...
\begin{theorem}[Qualitative determinacy]
...
%Formally, according to Theorem~\cref{theo:actionobservable},
%since both players can observe their actions,
%it is enough to prove that no behavioral
%strategy
...
%{
%According to Theorem~\cref{theo:actionobservable},
%since both players can observe their actions,
%it is enough to prove that no behavioral
%strategy
%$\sigma\in C^*\to \Delta(I)$ of Eve is almost-surely winning.
...
\subsection{Qualitative determinacy: proof of~\cref{8-thm:qualitative_determinacy}}
...
\begin{definition}[Maximal strategy]
...
{
Once $\sigma$ is fixed then the game is a one-player game with state space $V\times 2^V$ and imperfect information and~\cref{8-eq:pos2} implies that in this one-player game,
\be
...
Moreover~\cref{8-eq:belstab} implies that
...
According to~\cref{8-eq:sigmamproperty2},
...
\not=\emptyset$,~\cref{8-eq:postoprove} is obvious.  To deal with
...
$n=0$,~\cref{8-eq:topo} is obvious because $\KK_0=\emptyset$.  Suppose
that for some $n\in\N$, \cref{8-eq:topo} holds for every $L'\in\KK_n$,
...
to~\cref{8-eq:LLLn}, by definition of $\Phi$, there exists a signal
$d\in D$ such that $\action_2(d)=j_L$ and
 $\beldeux(L,d)\not \in \LL_n$ and $\beldeux(L,d)\neq \emptyset$.
{According to  property~\cref{8-eq:belief_compute} of beliefs (\cref{8-lem:beliefs}),} 
...
step.
{
Hence~\cref{8-eq:topo} holds for every behavioral strategy 
$\tau$. Thus, according to~\cref{8-lem:actioneq3},~\cref{8-eq:topo}
...
\enspace.
\]
The operator $\Psi$ is computable using as a nested fix-point the operator $\Phi$ of~\cref{8-thm:qdec1}.
