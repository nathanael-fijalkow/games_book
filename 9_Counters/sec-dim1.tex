\AP \Cref{12-th-undec} leaves open whether "vector games" might be
decidable in dimension one.  They are indeed decidable, and more
generally we learned in \cref{chap:pushdown} that "one-counter
...
"one-counter games" of \cref{chap:pushdown} only allow integer
...
\begin{corollary}\label{12-cor-dim1}
...
\end{corollary}
\begin{proof}\hfill\\
  \TODO{proof of \cref{12-cor-dim1} depends on how \cref{11-one-counter} is written}
\end{proof}
...
lower bound in \cref{12-one-counter}.  But first, we will study a
class of one-dimensional "vector games" of independent interest in
\cref{12-countdown}: "countdown games".
...
\begin{theorem}\label{12-countdown-given}
...
\begin{proof}
  ...
  arranged over a $t\times m$ grid as shown in \cref{12-fig-exp}.
  ...
  \begin{figure}[htbp]
    ...
    \hspace*{-.5ex}\begin{tikzpicture}[on grid,every node/.style={anchor=base}]
      ...
      \node at (9,-1.1) {$\blank$};      
      \end{tikzpicture}
    \caption{\label{12-fig-exp}The computation of~$\?M$ on
  ...
  than exhibiting the full computation from \cref{12-fig-exp}, the
  ...
  in \cref{12-fig-exp}, the $j$th cell of the $i$th
  ...
  of \cref{12-fig-exp} until she reaches the initial left endmarker:}
...
\begin{theorem}\label{12-countdown-exist}
  "Configuration reachability" and "coverability" "countdown games"
...
\begin{proof}
   ...
   the new system.  By \cref{12-cor-dim1}, this "configuration
   ...
   of \cref{12-countdown-given}, except that now the length~$t$ of the
...
\begin{theorem}\label{12-countdown-zero}
  "Zero reachability" "countdown games" with "given initial credit"
  are \EXP-complete.
\end{theorem}
\begin{proof}
  The upper bound of \cref{12-countdown-given} applies in the same
  way.  Regarding the lower bound, we modify the lower bound
  construction of \cref{12-countdown-given} in the following way: we
...
from \cref{12-cor-dim1} are tight in most cases.
\begin{theorem}\label{12-th-dim1}
...
\end{theorem}
\begin{proof}
  By \cref{12-countdown-exist}, "configuration reachability" and
  "coverability" "vector games" with existential initial credit
  are \EXPSPACE-hard in dimension one.
  Furthermore, \cref{12-cov2parity} allows to deduce that
  "parity@parity vector game" is also \EXPSPACE-hard.  Finally, we can
  argue as in the upper bound proof of \cref{12-countdown-exist} that
  ...
  of \cref{12-countdown-given,12-countdown-exist}: then the only way
