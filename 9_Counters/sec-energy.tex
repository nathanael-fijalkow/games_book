...
\begin{example}\label{12-ex-nrg}
  \Cref{12-fig-nrg} illustrates the "energy semantics" of the vector
  system depicted in~\cref{12-fig-mwg} on \cpageref{12-fig-mwg}.  Observe that,
  by contrast with the "natural semantics" of the same system depicted
  in \cref{12-fig-sem}, all the configurations $\loc'(0,n)$ controlled
...
\begin{figure}[thbp]
  ...
  \end{tikzpicture}}
  \caption{\label{12-fig-nrg} The "energy semantics" of the
...
\begin{proof}
  ...
  where we add the following locations controlled by~\Eve:
    ...
    \end{align*}
    \Cref{12-fig-avg} presents the result of this reduction on the
...
% case of dimension~one in \cref{12-sub-mono-dim1}.
