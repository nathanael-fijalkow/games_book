...
states", all the games presented in \cref{12-counters} are decidable.
...
time by~\cref{12-thm-zcycle} using linear programming
techniques~\cite{Kosaraju&Sullivan:1988}.

\paragraph{Undecidability of Vector Games}
The undecidability results of \cref{12-sub-undec} are folklore.  One
...
hardness proofs for \cref{12-countdown-given,12-countdown-exist} are
...
The "asymmetric" "vector games" of \cref{12-avag} appear
...
in~\cref{12-sub-reach} was already proven by \citem[Lincoln et
...
arguments like those of \cref{12-pareto-cov} was shown
...
By analysing the attractor computation of \cref{12-sub-attr}, one can
show that \cref{12-algo-cov} works in \kEXP[2], thus matching the
optimal upper bound from~\cref{12-avag-easy}: this can be done using
...
%% \cref{12-fact-nrg}, solving a multi-energy game is equivalent to
...
\paragraph{Complexity} \Cref{12-tbl-cmplx} summarises the complexity
results for "asymmetric vector games".  For the upper bounds with
"existential initial credit" of \cref{12-sub-up-exist}, the existence
...
"non-termination games"; the proof of \cref{12-counterless} is a
...
The \coNP\ upper of \cref{12-exist-easy} was shown soon after
...
\cref{12-exist-easy} to "parity@parity vector game" was shown
...
\cref{12-exist-easy} takes a slightly different approach using
\cref{12-lem-zcycle} for finding non-negative cycles, which is a
trivial adaptation of a result by \citem[Kosaraju and
Sullivan]{Kosaraju&Sullivan:1988}.  The pseudo-polynomial bound
of~\cref{12-exist-pseudop} is taken
from~\cite{Colcombet&Jurdzinski&Lazic&Schmitz:2017}.

For the upper bounds with "given initial credit" of
\cref{12-sub-up-given}, regarding "coverability", the \kEXP[2] upper
bound of \cref{12-avag-easy} was first shown by~\citem[Courtois and
...
of \cref{12-thm-zcycle}, from which the proof of \cref{12-counterless}
...
\cref{12-exist-hard} originates from
...
credit" from \cref{12-avag-hard} was shown
...
\cref{12-avag-two} by a reduction from "countdown games" was suggested
by~\citet{Mazowiecki&Perez:2017}.

The $\NP\cap\coNP$ upper bounds in dimension~one from
\cref{12-sub-mono-dim1} are due to \citem[Bouyer et
